{ pname, version, variant, src, patches ? _: [ ], meta }:
...
, imagemagick, jansson, libXaw, libXcursor, libXft, libXi, libXpm, libgccjit
, libjpeg, libotf, libpng, librsvg, libselinux, libtiff, libwebp, libxml2
, llvmPackages_14, m17n_lib, makeWrapper, motif, ncurses, nixosTests, pkg-config
...
, withSmallJaDic ? false, withCompressInstall ? true
...
, Accelerate, AppKit, Carbon, Cocoa, GSS, IOKit, ImageCaptureCore, ImageIO
, OSAKit, Quartz, QuartzCore, UniformTypeIdentifiers, WebKit }:
...
in mkDerivation (finalAttrs: {
  ...
  buildInputs = [ gettext gnutls harfbuzz.dev jansson libxml2 ncurses ]
    ...
    ++ lib.optionals withImageMagick [ imagemagick ] ++ lib.optionals withPgtk [
      giflib
      gtk3
      libXpm
      libjpeg
